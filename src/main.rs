...
fn do_it(args: CliArgs) -> Result<(), OurError> {
    ...
    let kit_dir_to_use = args
        .kit_dir
        .map_or_else(|| get_kit_dir(), |dir| std::path::PathBuf::from(dir));
    let bin_dirs = kits::get_kit_bin_dirs(kit_dir_to_use);

    let bin_dir_to_use = if let Some(kit_version) = args.kit_version {
        if let Some(found_dir) = bin_dirs
            .iter()
            .find(|dir| dir.file_name().unwrap().to_str().unwrap() == kit_version)
        {
            found_dir
        } else {
            // Get leaf folder name
            let latest_version = bin_dirs
                .last()
                .map(|dir| dir.file_name().unwrap().to_str().unwrap());
            return Err(OurError::BinDirNotFound {
                desired: kit_version,
                potential: latest_version.unwrap().to_string(),
            });
        }
    } else {
    ...
    match args.command {
        ...
        Commands::Tool {
            ...
            if subargs.list {
                ...
                println!(
                    " - {}",
                    "custom (use --custom-path to specify a path)".dimmed()
                );
                return Ok(());
            }
            ...
            if !tool_path.exists() {
                if allow_missing {
                    ...
                    return Err(OurError::ToolNotFound(
                        binary.to_string(),
            ...
            if run {
                ...
                let status = std::process::Command::new(&tool_path).status();

                match status {
                    Ok(s) => {
                        if !s.success() {
                            return Err(OurError::ToolFailed(
                                binary.to_string(),
...
mod tests {
    ...
    fn test_tool_not_found() {
        ...
        assert!(
            result.unwrap_err()
                == OurError::ToolNotFound(
                    "afakeexe.exe".to_string(),
                    bin_dir
                        .join("10.0.22000.0")
                        .join("x64")
                        .join("afakeexe.exe")
    ...
    fn test_version_not_found() {
        ...
        assert!(
            result.unwrap_err()
                == OurError::BinDirNotFound {
                    desired: "10.0.12345.0".to_string(),
                    potential: "10.0.22000.0".to_string()
                }
        );
    }
}
